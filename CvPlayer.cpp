...
{
	PROFILE_FUNC();
	//VINCENTZ TODO INSERT START GOLD BEFORE HEALING

	int goldbefore = getGold();
	...
	for (int iPass = 0; iPass < 4; iPass++)
	{
		...
		int UsedGold = goldbefore - getGold();
		if (getID() == GC.getGameINLINE().getActivePlayer() && UsedGold != 0)
		{
			//TODO INSERT GOLD
			gDLL->getInterfaceIFace()->addMessage(getID(), false, GC.getEVENT_MESSAGE_TIME(), gDLL->getText("TXT_KEY_MISC_GOLD_SPEND_HEALNG", UsedGold).GetCString(), "AS2D_PLAGUE", MESSAGE_TYPE_MINOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_WARNING_TEXT"));
