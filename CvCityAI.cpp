...
//						if (!bIsLimitedWonder || (iNumWaterPlots > NUM_CITY_PLOTS / 2))
                        if (!bIsLimitedWonder || (iNumWaterPlots > varNumPlots / 2))
						{
...
void CvCityAI::AI_updateBestBuild()
{
	...
	for (iI = 0; iI < varNumPlots; iI++)  // Mylon - Enhanced Sized Cities - Need to replace all instances of NUM_CITY_PLOTS
	{
	...
	for (iI = 0; iI < GC.getNumEmphasizeInfos(); iI++)
	{
		...
		if (AI_specialYieldMultiplier(YIELD_PRODUCTION) < 50)
		{
			...
			if (GC.getEmphasizeInfo((EmphasizeTypes)iI).isGreatPeople())
			{
				...
				int varNumPlots = getNumPlots(); // Mylon - Enhanced Sized Cities - Need to replace all instances of NUM_CITY_PLOTS
				for (int iPlot = 0; iPlot < varNumPlots; iPlot++)
				{
