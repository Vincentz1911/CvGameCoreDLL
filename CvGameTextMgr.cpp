	...
	else
	{
		...
		if (eRevealOwner != NO_PLAYER)
		{
			if (pPlot->isActiveVisible(true))
			{
				...
				if (pPlot->calculateCulturePercent(eRevealOwner) != 100) szString.append(szTempBuffer);
				//				szString.append(NEWLINE);
			...
			if (pPlot->calculateCulturePercent(eRevealOwner) != 100) szString.append(NEWLINE);
		}
...
//Vincentz #UPT		
		...
		//Moves
		//int iRegularCost = ((pPlot->getFeatureType() == NO_FEATURE) ? GC.getTerrainInfo(pPlot->getTerrainType()).getMovementCost() : GC.getFeatureInfo(pPlot->getFeatureType()).getMovementCost());
		int iRegularCost = GC.getTerrainInfo(pPlot->getTerrainType()).getMovementCost();

		if (pPlot->getFeatureType() != NO_FEATURE)
		{
			iRegularCost += GC.getFeatureInfo(pPlot->getFeatureType()).getMovementCost();
		}
		...
		szString.append(gDLL->getText("TXT_KEY_MOVECOST", iRegularCost));

		//IF ROAD
		if (pPlot->isRoute())
		{
			int iRouteCost = GC.getRouteInfo(pPlot->getRouteType()).getMovementCost() / GC.getMOVE_DENOMINATOR();
			szString.append(gDLL->getText("TXT_KEY_ROADMOVECOST", iRouteCost));
		}
				//std::max((GC.getRouteInfo(pFromPlot->getRouteType()).getMovementCost() + GET_TEAM(pUnit->getTeam()).getRouteChange(pFromPlot->getRouteType())),
		...
		//int Cost = 0;
		//Cost = pPlot->movementCost(pSelectionGroup->getHeadUnit(), pPlot); //VINCETEST
		//szString.append(gDLL->getText("TXT_KEY_UNITMOVECOST", Cost));
		//Moves end
		...
		//szString.append(szTempBuffer);
		int maxUPT = (GC.getDefineINT("MAXUPT") * GET_PLAYER(GC.getGameINLINE().getActivePlayer()).getCurrentEra() + GC.getDefineINT("STARTUPT"));
		szString.append(gDLL->getText("TXT_KEY_PLOT_MAXUPT", iNumVisibleUnits, maxUPT));
		...
		if (pPlot->isFreshWater() || pPlot->isLake())
		{
			//szString.append(NEWLINE);
...
void CvGameTextMgr::setGoodHealthHelp(CvWStringBuffer &szBuffer, CvCity& city)
{
	...
	if (city.goodHealth() > 0)
	{
		...
		iHealth = GC.getCultureLevelInfo(city.getCultureLevel()).getCultureHealth();
		if (iHealth > 0)
		{
